...
\begin{theorem}[Hausedorfův princip oddělení]\label{T-hausedorf}
    ...
    $\exists\delta>0$ tak, že $U(x_1,\delta)\cap U(x_2,\delta)=\text{\O}$

    Speciálně: $x_1\notin U(x_2,\delta);~x_2\notin U(x_1,\delta)$
\end{theorem}
\begin{proof}
    Volím $\delta=\frac{\abs{x_1-x_2}}{2}$. Tvrdím, že $U(x_1,\delta)\cap U(x_2,\delta)=\text{\O}$
...
\begin{proof}
    ...
    \begin{alignat}{1}
        \text{Věta \ref{T-hausedorf}}&\quad \exists\epsilon>0:
            U(A,\epsilon)\cap U(B,\epsilon)=\text{\O} \\
